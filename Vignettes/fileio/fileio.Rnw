...
\index{JCAMP-DX!Infrared} \index{Infrared!JCAMP-DX}
 Limited import of JCAMP-DX
files\cite{McDonald1988} is available in function \Rfunction{read.jdx}.  These files can contain
multiple spectra, supported data formats are tabular \texttt{XY..XY} and \texttt{X++(Y..Y)}.
<<>>=
read.jdx ("jcamp-dx/shimadzu.jdx", encoding = "latin1", keys.hdr2data=TRUE)

for (file in sprintf ("jcamp-dx/%s", c ("virgilio.jdx", "PE1800.DX", "LABCALC.DX")))
    print (read.jdx (file))
@
