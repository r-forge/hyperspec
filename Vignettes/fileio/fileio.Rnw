...
\index{JCAMP-DX!Shimadzu}
\index{Shimadzu!JCAMP-DX}
Rudimentary import of JCAMP-DX files\cite{McDonald1988} is available in function
\Rfunction{read.jdx.Shimadzu} which will read the JCAMP-DX files produced by the Shimadzu GCxGC-MS
library:

<<>>=
read.jdx.Shimadzu ("jcamp-dx/shimadzu.jdx", encoding = "latin1")
@ 

These files can contain multiple spectra, data format is \texttt{XY..XY} whith one X,Y pair per
line.
