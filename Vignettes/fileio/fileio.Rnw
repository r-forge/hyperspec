...
Witec also saves ASCII data with spectra in columns, see 
...
In the Witec project software, export the spectra as ASCII X and Y files. These can be read by \Rfunction{scan.dat.Witec}:

<<witec-dat>>=
scan.dat.Witec ("txt.Witec/WitecExample-x.dat", points.per.line = 10, lines.per.image = 10)
@

Another option is Witec's txt ASCII export, which produces ASCII files with each row corresponding to
