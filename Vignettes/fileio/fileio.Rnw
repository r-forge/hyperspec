...
for (file in sprintf ("jcamp-dx/%s", 
       c ("virgilio.jdx", "PE1800.DX", "LABCALC.DX", "AMA1.DX", "br_154_1.DX")))
    print (read.jdx (file))

lapply (Sys.glob ("jcamp-dx/PE-IR/*.DX"), read.jdx, ytol = 1e-8)
@
...
In the Witec project software, export the spectra as ASCII X and Y files (Save ASCII X and Save
ASCII Y) . These can be read by \Rfunction{scan.dat.Witec}:

<<witec-dat>>=
scan.dat.Witec ("txt.Witec/WitecExample-x.dat", points.per.line = 10, lines.per.image = 10)
@

Another option is Witec's txt table ASCII export (Export $\rightarrow$ Table), which produces ASCII files with each row corresponding to
one wavelength. Such files can be read with \Rfunction{scan.txt.Witec}:
<<witec-txt>>=
scan.dat.Witec ("txt.Witec/Witec.txt")
@ 

\Rfunction{scan.txt.Witec} assumes 1024 wavelengths, but other values may be given as parameter
\Rfunarg{nwl}. \Rcode{NULL} indicates that the number of wavelengths should be determined
automaticallly.
