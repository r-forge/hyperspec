...
@
\texttt{.spc} is in general the recommended format for \phy import, but this export option is not supported for imaging data. 

Imaging data can be exported as ASCII X and Y files (Save ASCII X and Save
