...
\item [{extra~data:}] spatial information (spectral images, maps, or profiles),
...
series, and
\item [{\Robject{laser}}] a time series of an unstable taser emission
\end{description}
In this vignette, all three data sets are used in an indermixed way
...
as an abbrevation for \emph{{}``column xxx of the data.frame in slot
data of the object''}.
...
%
\begin{table}
\noindent \centering{}\caption{\label{tab:Generic-methods}Generic methods implemented by \protect\Rclass{hyperSpec}
and some related functions. \emph{Emphasized} names indicate non-generic
functions that are closely related to the generic functions in the
row.}
\begin{small}\begin{tabular}{>{\raggedright}p{0.25\textwidth}>{\raggedright}p{0.6\textwidth}}
\toprule\textbf{Function} & \textbf{Explanation}\tabularnewline
\noalign{\vskip1em}
\cmidrule(lr){1-1}\cmidrule(lr){2-2} & \tabularnewline
\Rfunction{print}, \Rfunction{show}, \Rfunction{summary} & print information about the object\tabularnewline
\noalign{\vskip1em}
\Rfunction{plot} & plotting\tabularnewline
\noalign{\vskip1em}
\Rfunction{{[}{]}},\Rfunction{{[}{[}{]}{]}}, \Rfunction{\$} & extract parts of an object\tabularnewline
\noalign{\vskip1em}
\Rfunction{{[}<-}, \Rfunction{{[}{[}<-}, \Rfunction{\$<-} & assign parts of an object\tabularnewline
\noalign{\vskip1em}
\Rfunction{dim}, \Rfunction{ncol}, \Rfunction{nrow}, \emph{nwl} & the dimensions of the object\tabularnewline
\noalign{\vskip1em}
\Rfunction{colnames}, \Rfunction{rownames}, \Rfunction{dimnames} & names of the spectra, data columns, and both plus the names of the
wavelengths\tabularnewline
\noalign{\vskip1em}
\Rfunction{labels}, \emph{\Rfunction{\emph{labels<-}}} & labels for axis annotations etc.\tabularnewline
\noalign{\vskip1em}
\Rfunction{+ - {*} \textasciicircum{} \%\% \%/\% / \%{*}\%} & arithmetical operators work on \Rcode{\$spc}\tabularnewline
\noalign{\vskip1em}
\Rfunction{> < == >= <=} & comparison operators work on \Rcode{\$spc}\tabularnewline
\noalign{\vskip1em}
\Rfunction{log}, \Rfunction{log10}, \Rfunction{exp}, etc. & basic math functions work on \Rcode{\$spc}, see also \Rcode{? \textquotedbl{}hyperSpec Math\textquotedbl{}}\tabularnewline
\noalign{\vskip1em}
\Rfunction{min}, \Rfunction{max}, \Rfunction{range} & minimum, maximum, and range of the intensities in \Rcode{\$spc}\tabularnewline
\noalign{\vskip1em}
\Rfunction{cbind}, \Rfunction{rbind} & combine two objects by columns or rows\tabularnewline
\noalign{\vskip1em}
\Rfunction{split} & split an object into a list of objects\tabularnewline
\noalign{\vskip1em}
\Rfunction{apply} & apply a function row- or column-wise, calculate e.~g. the mean spectrum
or normalization factors\tabularnewline
\noalign{\vskip1em}
\Rfunction{aggregate}(\Rfunction{ave}) & calculate sum characteristics for groups of spectra, e.~g. cluster
mean spectra. \Rpackage{hyperSpec}'s \Rfunction{aggregate} method
covers also the functionality of \Rfunction{ave}.\tabularnewline
\noalign{\vskip1em}
\Rfunction{sweep} & {}``sweep'' a sum characteristic over rows or columns, e.~g. center
the data by substracting the mean spectrum.\tabularnewline
\noalign{\vskip1em}
\Rfunction{as.character}, \Rfunction{as.matrix}, \Rfunction{as.data.frame} & type conversion functions\tabularnewline
\noalign{\vskip1em}
\Rfunction{initialize} & create an object\tabularnewline
\noalign{\vskip1em}
\Rfunction{validObject} & validity checking\tabularnewline
\noalign{\vskip1em}
...
in many other programming languages. Functions may be added or changed
by the user in his \emph{workspace} at any time. Neither restart of
R nor reloading of the package or anything the like is needed. At
the same time, the original function is not deleted, it is just masked
by the user's new function but stays accessible if the change should
be reverted.

This offers the opportunity of easily writing specialized functions
...
stored in the object is appropriate for this class. In other words,
there is a mechanism of \emph{validity checking}.

The functions provided by \Rpackage{hyperSpec} do check the validity
of \Rclass{hyperSpec} objects at the beginning, and --- if the validity
could be broken by inapproprate arguments --- also before leaving
the function.
...
\section{The structure of \protect\Rclass{hyperSpec} objects}
...
%
\begin{figure}
\noindent \centering{}\include{Strukturhyperspec}\caption{\label{fig:structure}The stucture of the data in a \protect\Rpackage{hyperSpec}
object.}
...
or expressions for prettier axis annotations (see figure~\ref{fig:example-plots}).
To get familiar with expressions for axis annotition, see \Rcode{? plotmath}
and \Rcode{demo (plotmath)}.


\section{Obtaining Basic Information about \protect\Rpackage{hyperSpec} Objects}
...
\section{Combining \protect\Rclass{hyperspec} Objects%
\protect\marginpar{\protect\Rfunction{cbind} \protect\Rfunction{rbind}%
}}
...
\section{Creating a \protect\Rclass{hyperSpec} Object, Data Import and Export}


\subsection{ASCII Files%
\protect\marginpar{\protect\Rfunction{read.txt.long},\protect\Rfunction{read.txt.wide},\protect\Rfunction{write.txt.long},\protect\Rfunction{write.txt.wide}%
}}
...
optimized import function is already available: \Rfunction{scan.txt.Renishaw}.
%
\marginpar{\Rfunction{scan.txt.Renishaw}%
}
...
\subsection{Creating a \protect\Rclass{hyperSpec} Object from Spectra Matrix
and Wavelength Vector}

Once the data is in R's workspace, a \Rclass{hyperSpec} object is
created by \Rcode{initialize (\textquotedbl{}hyperSpec\textquotedbl{})}.
...
\section{Wavelengths and Spectral Axis}


\subsection{Wavelength Indices%
\protect\marginpar{\protect\Rfunction{wl2i}, \protect\Rfunction{i2wl}%
}}
...
\Rfunction{i2wl} returns the wavelength corrisponding to the given
...
to the corresponding wavelength. In addition there is a more sophioticated
...
and vectors%
\footnote{Formulas are combined to a list by \Rfunction{c}.%
}:
...
\section{Accessing the Extra Data }
...
\section{Accessing Parts of the \protect\Rclass{hyperSpec} Object%
\protect\marginpar{\protect\Rfunction{{[}{[}{]}{]} \$. \$..}%
}}

\Rclass{hyperSpec} comes with three abbrevation functions for easy
access to the data:
\begin{description}
\item [{\Rfunction{{[}{[}{]}{]}}}] returns the spectra matrix (\Rcode{\$spc}).
\item [{\Rfunction{{[}{[}\Rfunarg{i}, , \Rfunarg{l}{]}{]}}}] the cut spectr
amatrix is returned if wavelengths are specified is \Rfunarg{l}.
\item [{\Rfunction{{[}{[}\Rfunarg{i}, \Rfunarg{j}, \Rfunarg{l}{]}{]}}}] If
data columns are selected (second index), the result is a \Rclass{data.frame}.
\item [{\Rfunction{{[}{[}\Rfunarg{i}, , \Rfunarg{l}{]}{]}<-}}] Also, parts
of the spectra matrix can be set (only indices for spectra and wavelength
are allowed for this function). 
\item [{\Rfunction{{[}\Rfunarg{i}, \Rfunarg{j}{]}<-}}] sets parts of \Rcode{@data}.
\item [{\Rfunction{\$.}}] returns the complete \Rclass{data.frame} \Rcode{@data},
with the spectra in column \Rcode{\$spc}.
\item [{\Rfunction{\$..}}] returns the extra data (\Rcode{@data} without
\Rcode{\$spc}).
\item [{\Rfunction{\$..<-}}] sets the extra data (\Rcode{@data} without
\Rcode{\$spc}). However, the columns must be exactly tha same in
this case. 
\end{description}

\section{Selecting and Deleting Spectra}
...
argument. It may be a vector giving the indices of the spectra to
...
\begin{description}
\item [{\Rcode{plotspc}}] plots the spectra, i.~e. the intensities \Rcode{\$spc}
over the wavelengths \Rcode{@wavelength}.
\item [{\Rcode{plotmap}}] plots a false color map: a single value (e.\,g.
average intensity or cluster membership) over two data columns (default
\Rcode{\$x} and \Rcode{\$y}).
\item [{\Rcode{plotc}}] plots a time series or calibration plot: e.\,g.
an intensity over a single other data column (like concentration,
depth, or time).
\end{description}
All three plus some more handy abbrevations are also accessible via
\Rmethod{plot}:%
\marginpar{\Rfunction{plot}%
}
\begin{description}
\item [{\Rcode{plot (flu, \textquotedbl{}spc\textquotedbl{})}}] is equivalent
to \Rcode{plotspc (flu)}
\item [{\Rcode{plot (chondro, \textquotedbl{}spcmeansd\textquotedbl{})}}] plots
mean spectrum $\pm$ 1 standard deviation
\item [{\Rcode{plot (chondro, \textquotedbl{}spcprctl\textquotedbl{})}}] plots
median, 16$^{\text{th}}$ and 84$^{\text{th}}$ percentile. This is
similar to \Rcode{\textquotedbl{}spcmeansd\textquotedbl{}}. Spectroscopic
data frequently are not gaussian distributed. The percentiles give
a better idea of the true distribution. They are also less sensitive
to outliers.
\item [{\Rcode{plot (chondro, \textquotedbl{}spcprctl5\textquotedbl{})}}] like
\Rcode{\textquotedbl{}spcprctl\textquotedbl{}} plus 5$^{\text{th}}$
and 95$^{\text{th}}$percentile.
\item [{\Rcode{plot (chondro, \textquotedbl{}map\textquotedbl{})}}] is
equivalent to \Rcode{plotmap (chondro)}
\item [{\Rcode{plot (flu, \textquotedbl{}c\textquotedbl{})}}] is equivalent
to \Rcode{plotc (flu)}
\item [{\Rcode{plot (laser, \textquotedbl{}ts\textquotedbl{})}}] plots
a time series plot, equivalent to \Rcode{plotc (laser, use.c = \textquotedbl{}t\textquotedbl{})}
\item [{\Rcode{plot (x, \textquotedbl{}depth\textquotedbl{})}}] plots
a depth profile plot, equivalent to \Rcode{plotc (laser, use.c = \textquotedbl{}z\textquotedbl{})} 
\end{description}
...
\caption{\label{fig:example-plots}Some example plots. \subref{fig-plot-spc}\protect\Rcode{plotspc (flu)},
\subref{fig-plot-spcmeansd}\protect\Rcode{plot (chondro, \textquotedbl{}spcmeansd\textquotedbl{})},
\subref{fig-plot-c}\protect\Rcode{plotc (flu)}, and \subref{fig-plot-map}\protect\Rcode{plotmap (chondro)}.}

\end{figure}



\subsection{Plotting Spectra %
\protect\marginpar{\protect\Rfunction{plotspc}%
}}
...
\item [{in~different~colors}] colors use \Rcode{col = vector.of.colors}
...
}\caption{Arguments to \protect\Rfunction{plotspc}. \subref{fig-plotspc-ms}
\protect\Rcode{plot (fake.mass.spec, lines.args = list (type = \textquotedbl{}h\textquotedbl{}))}
\subref{fig-plotspc-paracetamol} \protect\Rcode{plotspc (paracetamol, wl.range = c (300 \textasciitilde{} 1800, 2800
\textasciitilde{} max), xoffset = 850, wl.reverse = TRUE)} \subref{fig-plotmap-factor} \protect\Rcode{plotmap} with a factor,
see section \ref{HCA}.}
...
\subsection{Calibration Plots, (Depth) Profiles, and Time Series Plots%
\protect\marginpar{\protect\Rfunction{plotc}%
}}

\Rfunction{plotc} plots an intensity over one of the extra data columns.
The abscissa uses column \Rcode{\$c} by default, another colum can
be specifiedusing \Rfunarg{use.c}\Rcode{= name}. The ordinate can
...
\subsection{Plotting False-Color Maps%
\protect\marginpar{\protect\Rfunction{plotmap}%
}}
...
\Rfunction{plotmap} produces a 3d plot, with the $z$ axis color-coded.
\Rfunction{plotmap}'s arguments \Rfunarg{x} and \Rfunarg{y} take
the name of extra data columns.


\paragraph{The color-coded axis. }

Also \Rfunarg{z}can be used to select one column of the extra data
by name. Alternatively, it may be a numeric or factor directy giving
the values to be used. Each level of a factor will have one color.
...
\subsection{Cutting the Spectral Range%
\protect\marginpar{\protect\Rfunction{{[}{]}}, \protect\Rfunction{{[}{[}{]}{]}}%
}}
...
\subsection{Spectral Interpolation and Smoothing%
\protect\marginpar{\protect\Rfunction{spc.bin}, \protect\Rfunction{spc.loess}%
}}
...
with inequal data point spacing should be interpolated onto an evenly
...
\captionbelow{\label{fig-spcloess}Smoothing interpolation by \protect\Rfunction{spc.loess}
...
with 2 cm\textsuperscript{-1} data point distance. This correspends
to a spectral resolution of about 4 cm\textsuperscript{-1}, and the
decrease in spectral resolution can be seen at tha sharp bands where
the maxima are not reached (due to the eact that the interpolation
...
\Rfunction{spc.fit.poly} fits a polynomual baseline of the given
...
fitted baselines. They need to be substracted afterwards:
...
each pixel. Such a constant can be immediately substracted:\\
\Rcode{spectra - constant}


\subsubsection{Correcting Wavelength Dependence}

This means that for each of the wavelenths the same correction needs
...
be 1:
\begin{enumerate}
\item Pixel dependent offsets are substracted:\\
...
\Rfunction{mean} gives fqual results, just that tha Intensities are
...
thereby normalizing on a particalar band or peak.
...
Centering means that the mean spectrum is substracted from each of
...
It is perfectly fine to center the \Robject{flu} data set: the inderpretation
is that centering he data cancels the offset (background spectrum
...
spectra, so the spectroscopic interpretiation of centering is getting
...
sample? In this case it is better rather want to substract the minimum
...
the influence and scalung of the variates (that are typically different
physical values). However, it is hardly appropriate for spetra that
do have the same scale of the same physical value.


\subsection{Multiplicative Scatter Correction (MSC)}

MSC can be done using \Rfunction{msc}from package \Rpackage{pls}\citep{Mevik2007}.
...
\subsection{Spectral Arithmetic%
\protect\marginpar{\protect\Rfunction{+ - {*} / \textasciicircum{} log log10}%
}}
...
\Rcode{labels (absorbance.spectra)\$spc <- \textquotedbl{}A\textquotedbl{}}%
\marginpar{\Rfunction{labels}%
}\\
...
of the correct size.\\
Matrix multiplication is done by \Rfunction{\%{*}\%}%
\marginpar{\Rfunction{\%{*}\%}%
}, again each of the operands may be a matrix or a \Rclass{hyperSpec}
...
\subsection{Data Analysis Methods using a data.frame\protect \\
e.\,g. Principal Component Analysis with \protect\Rfunction{prcomp}}
...
\subsection{Data Analysis Methods using a matrix\protect \\
...
dendrogram \subref{fig:clust:mab} the map of the 3 clusters \subref{fig:clust:spc}
the mean spectra.}
...
\subsection{Calculating group-wise Sum Characteristics\protect \\
...
\bibliography{/home/cb/Uni/Literatur/Literatur}
