...
\newcommand{\phy}{\Rpackage{hyperSpec}\xspace}
...
Table~\ref{tab:generic} and \ref{tab:functions} give an overview of the functions implemented by \phy. 
\input{tab-generic}
\input{tab-functions}
\ref{fig:schema} is a reference card providing an overview which functions help to accomplish what (spectroscopic) task. 
\begin{figure}[p]
  \includegraphics[width=\textwidth]{hyperSpec-schema-annotated}
  \caption{Overview of functions to accomplish common spectroscopic tasks.}
  \label{fig:schema}
\end{figure}
...
There is indication that the position of the first spectrum is recorded in μm, while the pixel size is in mm. Thus a flag \Rfunarg{nicolet.correction} is provided that divides the pixel size by 1000. 
...
This is done by  \Rfunction{plotmat}, which is set up similar to \Rfunction{plotmap} and \Rfunction{plotc}. Argument \Rfunarg{use.t} specifies the extra data column to use as ordinate, and defaults to "t". The plot can be conditioned by giving the conditioning term in \Rfunarg{cond}. The actual plot is produced by  \Rfunction{levelplot}, and further arguments are handed over directly. See figure~\ref{fig:plotmat}
<<plotmat1, fig = TRUE, include = FALSE>>=
print (plotmat (laser, col.regions = matlab.palette (20)))
@
<<plotmat2, fig = TRUE, include = FALSE>>=
plot (plotmat (flu, use.t = "c", contour = TRUE, labels = TRUE, col = "#00000080"))
@
\begin{figure}
\subfloat[\label{fig-plotmat-a}\Rcode{plotmat (laser, col.regions = matlab.palette (20))}]{\includegraphics[width=0.495\textwidth]{introduction-plotmat1}}%
\subfloat[\label{fig-plotmat-b}\Rcode{plotmat (flu, use.t = "c", contour = TRUE, labels = TRUE, col = "#00000080")}]{\includegraphics[width=0.495\textwidth]{introduction-plotmat2}}%
\caption{\Rfunction{plotmat}: plotting the spectra matrix. 
}
\end{figure}
