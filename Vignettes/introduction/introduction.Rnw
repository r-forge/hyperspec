...
Assume \code{data} is the data cube, and \code{x}, \code{y} and \code{wl} hold vectors with the proper $x$ and $y$ coordinates and the wavelengths:
<<array>>=
data <- array (1 : 24, 4 : 2)
wl <- c (550, 630)
x <- c (1000, 1200, 1400)
...
If no proper coordinates (vectors \code{x}, \code{y} and \code{wl}) are available, they can be left
out. In the case of $x$ and $y$, map plotting will then be impossible, missing \Rfunarg{wavelength}s
will be replaced by column indices counting from \code{1} to \code{d [3]} automatically. Of course,
such sequences (the row/column/pixel numbers) can be used instead of the original \code{x} and
\code{y} as well:
<<>>=
y <- seq_len (d [1])
x <- seq_len (d [2])
