...
\phy is a R package that allows convenient handling
...
However, \phy works perfectly fine with any data
...
the R package \phy. It comes with five data sets,
\begin{labeling}{wavelength: }
...
\phy, it is organized more closely on spectroscopic
...
the table in appendix~\ref{tab:functions} on page~\pageref{tab:functions}.
...
The functions provided by \phy check the validity of \Rclass{hyperSpec} objects at the beginning, and --- if the validity could be broken by inappropriate arguments --- also before leaving the function.
...
Operators such as \verb/+/,  \verb+*+, \verb+%%+, etc. are in fact functions in R. Thus they can be handed over as arguments to other functions (particularly to the vectorization functions \Rfunction{*apply}, \Rfunction{sweep}, etc.). In this case the name of the function must be quoted: \verb+`*`+ is the recommended style (although \verb+"*"+ will often work as well), \eg:
<<eval = FALSE>>=
sweep (flu, 2, mean, `-`)
@
...
@
The actual name of the function is \verb+wl<-+ and must be quoted in order to avoid confusion with an assignment to variable \verb+wl+: \verb+`wl<-`+.
...
The global behaviour of \phy can be configured via options. The values of the options can be
retrieved with \Rfunction{hyperSpec.getOptions} and \Rfunction{hyperSpec.getOption}, and changed with
\Rfunction{hyperSpec.setOptions}.  

Currently, the only option provided is \Robject{log}, a logical specifying whether assignment
functions should automatically add entries to the logbook (see \ref{sec:logbook}).

\section{The structure of \Rclass{hyperSpec} objects}
\label{sec:structure-chy}

\Rclass{hyperSpec} is a S4 (or new-style) class. It has four so-called \emph{slots} that contain
parts of the object:
\begin{labeling}{@wavelength }
\item [{\Robject{@wavelength}}] containing a numeric vector with the wavelength axis of the spectra.
\item [{\Robject{@data}}] a \Rclass{data.frame}with the spectra and all further information belonging
  to the spectra
\item [{\Robject{@label}}] a list with appropriate labels (particularly for axis annotations)
\item [{\Robject{@log}}] a \Rclass{data.frame} keeping track of what is done with the object
\end{labeling}
However, it is good practice to use the functions provided by \phy to handle the
objects rather than accessing the slots directly (tab.~\ref{tab:getset}). This also ensures that
proper (\emph{valid}) objects are retained.
\begin{table}[t]
  \centering
...
Most of the data is stored in \Robject{@data}. This \Rclass{data.frame} has one special column,
\Rcode{\$spc}. It is the column that actually contains the spectra. The spectra are stored in a
matrix inside this column, as illustrated in figure~\ref{fig:structure}. Even if there are no
spectra, \Rcode{\$spc} must still be present. It is then a matrix with zero columns.
\begin{figure}[bt]
\noindent \centering
...
label for the wavelength axis, \Rcode{.wavelength}. The elements of the list may be anything suitable
for axis annotations, \ie they should be either character strings or expressions for ``pretty'' axis
annotations (see figure~\ref{fig-spcloess} on page~\pageref{fig-spcloess}).  To get familiar with
expressions for axis annotation, see \verb+? plotmath+ and \verb+demo (plotmath)+.
...
of the spectra matrix are the wavelengths. They are accessed by \Rfunction{wl}, see
section~\ref{sec:wavel-axis-conv}.
...
document. Use \verb+vignette ("file-io")+ to read about import and export of spectra into \chy
objects.
...
\section{The Logbook}
\label{sec:logbook}
Slot \Rcode{@log} of \chy objects is intended to keep track of the history of the object. 

This logbook part of the output of the \Rfunction{summary}, and can also be retrieved by
\Rfunction{logbook}. 
...
\mFun{logentry, logbook}
New entries can be created manually by calling \Rfunction{logentry}:
<<>>=
...
by columns to append a new spectral range or by row to append new spectra

<<cbind>>=
dim (flu)
dim (cbind (flu, flu))
dim (rbind (flu, flu))
@ Thus, you can use \Rfunction{cbind} to add new spectral ranges, and \Rfunction{rbind} to add new
spectra to your object.
...
The \Robject{barbituates} data is a list of \Sexpr{length (barbituates)} \chy objects, each
containing one mass spectrum. The spectra have \Sexpr{(min (sapply (barbituates, nwl)))} to
\Sexpr{(max (sapply (barbituates, nwl)))} data points. As a second step, the resulting object's
wavelelength axis is sorted:
<<collapse>>=
...
works even if spectra are missing in one of the objects. 

The arguments \Rfunarg{by}, \Rfunarg{by.x}, and \Rfunarg{by.y} specify which columns should be used
to decide which spectra are the same. 

The arguments \Rfunarg{all}, \Rfunarg{all.x}, and \Rfunarg{all.y} determine whether spectra should be
kept for the result set if they appear in only one of the objects.

For details on \Rfunction{merge}, see the help on \Rpackage{base::merge}.

As an example, let's construct a version of the \Robject{chondro} data like being taken as two maps with
different spectral ranges. In each data set, some spectra are missing.
<<merge-sample>>=
...
nrow (chondro.merged)
@
By default, the result consists of only those spectra, where \emph{both} spectral ranges were available.
To keep all spectra, regardless of the other part, use:
<<>>=
...
<<missing, fig = TRUE, width = 10, height = 5, echo = FALSE>>=
print (levelplot (spc ~ x * y | as.factor (paste (.wavelength, "  1/cm")), chondro.merged [,,c(1000, 1650)], 
           aspect = "iso", col.regions = matlab.palette ()))
@ 
}
\setkeys{Gin}{width = .33\textwidth}
\subfloat[\label{fig-merge-mat}]{
<<merged, fig = TRUE, width = 5, height = 5, echo = FALSE>>=
print (plot (chondro.merged, "mat", col.regions = matlab.dark.palette ()))
rm (chondro)
@ 
}
...
If the spectra overlap, the result will have both data points. The appropriate method to deal with
this situation depends on the data at hand. One possibility, however, is removing duplicated
wavelength by using the mean intensity. This can conveniently be done by using \Rfunction{approx}
using \Rcode{method = "constant"}.  For duplicated wavelengths, the intensities will be combined by
the tie function. This already defaults to the mean, but we need it with \Rcode{na.rm = TRUE}.
...
result of \Rfunction{[[]]} is a data.frame if extra data columns were selected or otherwise the
...
\Rfunction{\$<-}\mFun{\Rfunction{[]<-}, \Rfunction{[[]]<-}, \Rfunction{\$<-}} (see~).
...
(extra) data columns. They can be given like any column specification for a \Rclass{data.frame}, \ie
...
@
\\
(It is possible to produce a plot of this data where the cut range is not bridged by a line and the
wavelength axis is cut in order to save space. For details see the ``plotting'' vignette).
...
in \phy have always discretized wavelength
axes, they are stored in a matrix with column corresponding to one
wavelength. \phy provides two conversion functions:
...
Sometimes wavelength axes need to be transformed, \eg converting from wavelengths to frequencies. In
this case, retrieve the wavelength axis vector with \Rfunction{wl}\mFun{\Rfunction{wl},
  \Rfunction{wl<-}}, convert each value of the resulting vector and assign the result with
\Rfunction{wl<-}. Also the label of the wavelength axis may need to be adjusted.
...
\subsection{More on the Square-Bracket Operators for  Replacing Values}
\label{sec:square-brack-replace}
\Rfunction{[[]]} also accepts index matrices of size $n \times 2$. In this case, a vector of values from the spectra matrix is returned.
<<data2>>=
...
\item [\Robject{x} \Rfunction{[[\Rfunarg{i}, \Rfunarg{j}, \Rfunarg{l}]]}] If data columns are
  selected (second index), the result is a \Rclass{data.frame}.
...
\item [\Robject{x} \Rfunction{\$.}] returns the complete \Rclass{data.frame} \Rcode{x@data}, with the
...
\item [\Robject{x} \Rfunction{\$.. <-}] sets the extra data (\Rcode{x@data} without
  \Rcode{x\$spc}). However, the columns must match exactly in this case.
\end{description}

\subsection{Conversion to Long-Format data.frame}
\label{sec:conv-long-form}
Some functions need the data being an \emph{unstacked} or \emph{long-format}
data.frame. \mFun{\Rfunction{as.long.df}}\Rfunction{as.long.df} is the appropriate conversion
function.
...
(or the data.frame \Rcode{@data} if in addition data columns were specified) only.
...
best with spectrally oversampled data that allows accurate deterimnation of the signal position.
...
df <- array2df (cbind (shifts, shifts + shift1, shifts + shift2), 
                levels= list (NULL, method = c ("original", "find maximum", "interpolation")),
                label.x = "shift")
...
\mFun{\Rfunction{pls::msc}}MSC can be done using \Rfunction{msc} from package \Rpackage{pls}\citep{Wehrens2007}.
...
\subsection{Data Analysis using long-format data.frame\\ \eg plotting with ggplot2}
\label{sec:as.long.df}
Some functions need the data being an \emph{unstacked} or \emph{long-format} data.frame. \mFun{\Rfunction{as.long.df}}\Rfunction{as.long.df} is the appropriate conversion function.
<<>>=
library (ggplot2)
...
In that case, the calculations may be sped up considerably if the required parts of the \chy object
are extracted into a data.frame or matrices beforehand. A related tip is that that many model fitting
functions in \R are much faster if the formula interface is avoided and the appropriate data.frames
or matrices are handed over directly. 

Another possibility to speed up is to switch of the automatic logging of how the objects are
transformed. Logging involves appending rows to the data.frame in slot \Rcode{@log}.  While the
absolute amount of time needed to add a logbook entry is small, it may be executed very often
(e.g. during each call of \Rfunction{[}). However, the first strategy will usually allow for far
larger gains in speed.

As an example, let's again consider the code for shifting the spectra:
<<>>=
system.time ({
  for (i in seq_len (nrow (chondro)))
    chondro [[i]] <- interpolate (tmp [[i]], shifts [i], wl = wl (chondro))
})
rm (chondro)

hyperSpec.setOptions (log = FALSE)
system.time ({
  for (i in seq_len (nrow (chondro)))
    tmp [[i]] <- interpolate (tmp [[i]], shifts [i], wl = wl (chondro))
})
hyperSpec.setOptions (log = TRUE)
rm (chondro)
