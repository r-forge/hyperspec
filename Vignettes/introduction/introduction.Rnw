...
mean and covariance matrix using \Rfunction{rmvnorm}\mFun{pooled.cov} (fig.~\ref{fig:sim:spc}). Mean
spectra and pooled covariance matrix can be calculated using \Rfunction{pooled.cov}\mFun{pooled.cov}:
...
@ 
\begin{figure}[t]
\subfloat[\label{fig:sim:spc} \Rfunction{rmvnorm}]{\includegraphics[width=.66\linewidth]{introduction-fig--simspc}}
...
<<lda>>=
require ("MASS")
rnd <- rmvnorm (rep (200, 3), mean = pcov$mean, sigma = pcov$COV)
