...
A first possibility to speed up is switching of the automatic logging of how the objects are
...
beforehand. A related tip is that that many model fitting functions in \R are much faster if the
formula interface is avoided and the appropriate \df{}s or matrices are handed over directly. 
<<speed3>>=
tmp <- chondro [1 : 50]
system.time ({
  tmp.matrix <- tmp [[]]
