...
\mFun{colnames, rownames, dimnames, \emph{wl}} 
...
Extra data column names and rownames of the object may be set by \Rfunction{colnames<-} and \Rfunction{rownames<-}, respectively. \Rfunction{colnames<-} renames the labels as well.
...
\mFun{\Rfunction{collapse}}
\subsection{Binding Objects that do not share the same extra data and/or wavelength axis}
\label{sec:collapse}

\Rfunction{collapse} combines objects that should be bound together by row, but they do not share the columns and/or spectral range.
...
Two \chy objects can be matrix multiplied by \Rfunction{\%*\%}. For an example, see the principal component analysis below (section~\ref{sec:pca} on page \pageref{sec:pca}).
...
The main functions to retrieve the data of a \chy object are \Rfunction{[]} and \Rfunction{[[]]}. 

\subsection{Selecting and Deleting Spectra}
\label{sec:select-delete-spectra}

The extraction function \Rfunction{[]} (or \Rfunction{[[]]}, if the spectra \Rclass{matrix} or the \Rclass{data.frame} is needed rather than a \Rclass{hyperSpec} object) takes the spectra as first argument (For detailed help: \Rcode{? \textquotedbl[\textquotedbl}). It may be a vector giving the indices of the spectra to extract (select), a vector with negative indices indicating which spectra should be deleted, or a logical. Note that a matrix given to  \Rfunction{[]} will be treated as a vector.
...
\subsection{The Square-Bracket Operators}
\label{sec:square-brack-oper}

In addition, \Rfunction{[[]]} accepts index matrices of size $n \times 2$.
<<data2>>=
