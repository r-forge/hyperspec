...
functions should automatically add entries to the logbook (see \ref{sec:structure-chy} and
\ref{sec:speed-considerations} on page \pageref{sec:speed-considerations}).

The default behaviour is to automatically log changes to the object. As the automatic logging
mechanism can only log function calls and parameters (as opposed to the intention of the function
call), you may obtain a more useful log by switching off the automatic logging and 
...
Results of such a \mFun{\Rfunction{decomposition}} decomposition can be put again into \Rclass{hyperSpec} objects. This allows to plot \eg the loading like spectra, or score maps, see figure~\ref{fig:pca}.
...
In that case, one possibility to speed up is to switch of the automatic logging of how the objects
are transformed. Logging involves appending rows to the data.frame in slot \Rcode{@log}.  While the
