...
\indent If you use \phy with data formats not mentioned in this document, please send an email to the
maintainer, so that this document can be updated. 
...
If you need help finding out how to import your data, feel free to ask, preferrably via the mailing
list or feature request tracker on \phy's home page
...
specialized import functions. These are in general named after the data format followed by the
manufacturer (\eg read.ENVI.Nicolet).

Finally, we give overview lists of the directly supported file formats: sorted by file format
...
more than welcome to contribute such import code to \phy. Secion~\ref{sec:writing-Import} gives an
example of how to wrap up the code.
...
As we experienced missing header files (Bruker's Opus software frequently produces header files
...
Thermo Galactic's .spc file format\cite{Galactic1997} can be read by \Rfunction{read.spc}. 

A vast variety of sub-formats is available. \phy's import function does not support the \emph{old
file format} that was used before 1996. In addition, no test data with \emph{w planes} was available
...
This information is often useful (Kaiser's Hologram software \eg stores ).

\Rfunction{read.spc} offers four arguments that allow fine-grained control of storing such information in the \chy object. 
\begin{labeling}{\Rcode{keys.hdr2data}}
\item[\Rcode{keys.hdr2data}] parameters from the spc file header and subfile header that should become extra data columns
\item[\Rcode{keys.hdr2log}] parameters from the spc file header and subfile header that should be stored as list entries in the \Rfunarg{long.description} of the log entry
...
\end{labeling}
The value of these arguments can either be logical or a character vector of parameter names. Note that the header file field names are always lowercase, while the log entries are treated case sensitive.
...
such import filters to \Rpackage{hyperSpec}: send me 
(\href{mailto:cbeleites@units.it}{cbeleites@units.it})
...
header file that should allow to reconstruct the lateral coordinates as well as the spectral axes'
units. \phy has a specialized function \Rfunction{read.ENVI.Nicolet} that uses these header entries.
...
\subsubsection{A new ASCII Import Function: scan.txt.PerkinElmer}
...
scan.txt.PerkinElmer ("txt.PerkinElmer/*.txt", skip = 54)
@
imports the spectra.
...
\subsubsection{Deriving a More Specific Function: read.ENVI.Nicolet}
<<echo = FALSE, results = tex>>=
make.all.permutations ("ENVI", "Infrared", "Nicolet")
