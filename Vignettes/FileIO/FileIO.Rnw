...
\item Spectrometer model and manufacturer
\item The ``native'' file format
\item Relevant procedures to convert the file
\item R code to import the data
\item Documentation, particularly the description of the data format (if available)
\end{itemize}
...
Like \Rfunction{read.ENVI}, \Rfunction{read.spc} allows to use pre-defined header values. See the
help for details.
...
\subsection{Kaiser Optical Systems Raman Maps}
<<echo = FALSE, results = tex>>=
...
filter for an ASCII file format basically from scratch. 
...
\begin{tabular}{llllp{3.4cm}}
  \textbf{Format} & \textbf{Manufacturer} & \textbf{Function} & \textbf{section}  & \textbf{Notes}\\[1em]
...
\section{File Import by Functions  Manufacturer}
\label{sec:manufacturer}
\noindent
\begin{footnotesize}
\begin{tabular}{llllp{3.4cm}}
  \textbf{Manufacturer} & \textbf{Format} & \textbf{Function} & \textbf{section}  & \textbf{Notes}\\[1em]
...
\noindent
\begin{footnotesize}
\begin{tabular}{lllllp{3.4cm}}
  \textbf{Spectroscopy} & \textbf{Format} & \textbf{Manufacturer} & \textbf{Function} & \textbf{section}  & \textbf{Notes}\\[1em]
<<results=tex, echo = FALSE>>=
i <- order (fileformats$Spectroscopy, tolower (fileformats$Name))
res <- apply (fileformats [i,], 1, 
       function (x) {
