...
\Rfunction{readMat} imports the .mat file's contents into a list. The \chy object can be created using \Rfunction{new}, see~\ref{sec:new}~(p.–\pageref{sec:new}).
...
\phy's function \Rfunction{read.ENVI} reads ENVI files. ENVI files are binary data accompanied by an ASCII header file.
