...
print (plotmap (chondro, clusters ~ x * y, col.regions = c ("dark blue", "orange", "dark red")))
@
...
points (seq_along (dendrogram$order), rep (-3, length (dendrogram$order)),
        col = col.clust [chondro$clusters [dendrogram$order]], pch = "|")
@     
...
plot(cluster.means, stacked = ".aggregate", fill = ".aggregate", 
     col = c ("dark blue", "orange", "dark red"))
@
