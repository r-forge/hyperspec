% -*- mode: noweb; noweb-default-code-mode: R-mode; noweb-doc-mode: latex-mode; -*-
...
The function should automatically read in all files specified by a pattern, such as \texttt{*.txt}.
...
with the special name \Rcode{.wavelength} corresponds to the wavelength axis, all data columns should
have a label with the same name. The spectra are always in a data column called \Rcode{spc}.

<<importfun>>=
read.PE <- function (files = "flu?.txt", skip = 54) {
...
(also via \Rfunction{predict}) together with the prediction example:
