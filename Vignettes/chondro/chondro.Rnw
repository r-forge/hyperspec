...
\warnbox{Reproducing this vignette}{ The data set and source file of this vignette are available as a
  ...
  \url{http://hyperspec.r-forge.r-project.org/chondrocytes.zip} (ca. 9 MB).  The original data file
...
\caption{\label{fig:vis-all}Microphotograph of the cartilage section. The frame indicates the
  measurement area (\mum{35~\times~21}).}
\end{figure}
...
\item [Measurement grid:] 35 $\times$21 \mum{}, 1 \mum{} step size
...
spectra (\Rcode{data = \textquotedbl{}spc\textquotedbl{}}), time series (\Rcode{data =
  \textquotedbl{}ts\textquotedbl{}}), and depth profiles (data =
...
spectra.]{\includegraphics[width=0.5\textwidth]{fig/fig-rawspc}}
\subfloat[\label{fig:raw:b}[The sum intensity of the raw spectra.]{\includegraphics[width=0.5\textwidth]{fig/fig-rawmap}}
...
introduces a collinearity. This can be broken by deleting one wavelength:

<<>>=
chondro <- chondro [,, min ~ max - 1i]
@
...
png ("fig/fig-pca-pairs2.png", width = 500, height = 500)
...
\subfloat[The suspected outlier spectra.]{\includegraphics[width=0.495\textwidth]{fig/fig-pca-outspc}}
\caption{Outlier removal by PCA}
\end{figure}
...
\end{figure}
\section{Classification: Linear Discriminant Analysis (LDA)}
\label{sec:line-discr-analys}
Another typical task in spectroscopic data analysis is classification.  Classification uses reference
label in addition to the spectra. Let's use the results of the cluster analyis in the previous
section as (surrogate) reference.

We can set up a classification model via \Rpackage{MASS}'s function \Rfunction{lda}. Normalizing the
data set created collinearity between our variates. Thus we can omit one variate (e.g. the last). In addition, we also omit the outliers:
<<>>=
library (MASS)
lda <- lda (clusters ~ spc, data = chondro$.)
@

We can now study the samples in LDA space (similar to a PCA scores plot, plot not shown):
<<fig = FALSE>>=
plot (predict (lda)$x, col = cols [chondro$clusters], pch = 19, cex = 0.75)
@ 
%
In order to gain interpretability, the LDA model can be rotated and flipped, so that e.g. the
``cell'' class lies directly right of the ``lacuna'' class. Rotating and flipping does not change
the predictions of an LDA model:

<<rotateLDA>>=
## rotation matrix for given angle
rotmat <- function (alpha){ 
  matrix (c (cos (alpha), -sin (alpha), 
             sin (alpha),  cos (alpha)), 
          nrow = 2, byrow = TRUE)
}

meanscore <- lda$means %*% coef (lda)
tmp <- meanscore ["cell", ] - meanscore ["lacuna", ]
alpha <- atan (tmp [2] / tmp [1])

rotation.matrix <- rotmat (alpha)
@
We rotate the model \Sexpr{format (alpha * 180 / pi, digits = 3)}\textdegree (counterclockwise) and put the results into derived \chy objects:
<<>>=
coef.rot <- decomposition (chondro, t (coef (lda) %*% rotation.matrix), scores = FALSE)
scores.rot <- decomposition (chondro, predict (lda)$x %*% rotation.matrix)
@
The results are in fig.~\ref{fig:LDArot}.
<<ldascores, fig = TRUE, width = 5, height = 5, include = FALSE>>=
plot (scores.rot [[]],
      col = cols [scores.rot$clusters], 
      pch = 19, cex = 0.75, asp = 1)
@

\begin{figure}[tbh]
\subfloat[\label{fig:LDAscores} The rotated scores.]{
  \includegraphics[width=0.33\textwidth]{fig/fig-ldascores}
}%
\subfloat[\label{fig:LDAcoef} The coefficient spectra.]{
  \includegraphics[width=0.33\textwidth]{fig/fig-ldacoef}
}
\subfloat[\label{fig:LDAcontrib} The contibution spectra.]{
  \includegraphics[width=0.33\textwidth]{fig/fig-ldacontrib}
}
\caption{Classification: the rotated LDA model.}
\label{fig:LDArot}
\end{figure}
<<ldacoef, fig = TRUE, width = 5, height = 5, include = FALSE>>=
plot (coef.rot, stacked = TRUE)
@


\subsection{Contribution spectra}
\label{sec:contribution-spectra}
For spectroscopic interpretation, the coefficients alone may not be sufficient and are possibly
uninterpretable (see fig.~\ref{fig:LDAcoef}): high coefficient for variates that have low intensities
may contribute only minutely to the final classification and vice versa. However, the scores are
calculated by matrix multiplication of the spectra with the coefficients. By element-wise
multiplication without summation, we can calculate how much each variate contributes to the final
score (fig.~\ref{fig:LDAcontrib}):
<<>>=
contributions <- lapply (1 : 2, 
   function (i) sweep (chondro, 2, coef.rot [[i,]], `*`)
)
...
@ 
%
Now the used spectroscopic information becomes clear again, e.g. the lipid bands used to
distinguish matrix from lacun\ae and cells is prominent in the contribution spectra of LD 2 for the
matrix class.

<<ldacontrib, fig = TRUE, width = 5, height = 5, include = FALSE>>=
plot (tmp, stacked = ".aggregate", fill = ".aggregate", col = rep(cols, 2))
@
