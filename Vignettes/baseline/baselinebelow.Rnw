...
<<echo = FALSE, results = hide>>=
	trace (spc.fit.poly.below, quote ({ls ();
						plot (fit.to[,, use.old], col = cl, add = TRUE, lines.dots = list (pch = pch, type = "p")); 
...
plot (chondro [1,,1700:1800], plot.dots = list (ylim = c (-50, 200)))
cl <- "black"
pch = 1
bl <- spc.fit.poly.below (chondro [1,,1700:1800], poly.order = 1)
pch = 20
plot (chondro [1,,1715:1800], col = "blue", add = TRUE, lines.dots = list (lwd = 2))
cl <- "blue"
bl <- spc.fit.poly.below (chondro [1,,1715:1800], poly.order = 1)
@
\begin{figure}[b]
\begin{center}
...
system.time (spc.fit.poly.below (chondro [,, -wl2i (chondro, 700:1700), index = TRUE], NULL, 
...
plot (chondro [1], lines.dots = list (type = "n"))
...
<<fig2, fig=TRUE, pdf = TRUE, echo = FALSE, results = hide, eps = FALSE, width = 12, height = 2.5, include = FALSE>>=
	bl <- chondro [1] + 1
	plot (chondro [1], plot.dots = list (ylim = c (-15, 50)))
	for (iter in seq_along (cols)){
...
noise <- 12
	plot (chondro [1,,1730:1800], plot.dots = list(ylim = c(-15, 20)))
	trace (spc.fit.poly.below, quote ({
						plot (fit.to[,, use.old], col = cl, add = TRUE, lines.dots = list (pch = 20, type = "p")); 
						lines (fit.to@wavelength, bl, col = cl);
	...
	bl <- chondro [1,,1730:1800] + 15
	for (iter in seq_along (cols)){
		npts <- sum (chondro [[1,,1730:1800]] < (bl [[]] + noise))
		cl <- cols [iter]
		bl <- spc.fit.poly.below (chondro [1,,1730:1800], poly.order = 1, npts.min = npts, noise = noise)
		text (1800, 20 - 3 * iter, paste ("Iter. ", iter, ": ", npts, " support pts.", sep = ""), 
				adj = c(1, 0), col = cols [iter])
	}
	cl <- "black"
	trace (spc.fit.poly.below, quote ({
						plot (fit.to[,, use.old], col = cl, add = TRUE, lines.dots = list (type = "p")); 
						lines (fit.to@wavelength, bl, col = cl);
					}), at = 10, print = FALSE)
	bl <- spc.fit.poly.below (chondro [1,, 1710:1800], poly.order = 1)
@
