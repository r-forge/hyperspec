...
\Rcode{spc.fit.poly.below (fit.to, apply.to = fit.to, poly.order = 1, npts.min = NULL, noise = 0,\\
...
\item[\Rfunction{apply.to}:] \Robject{hyperSpec} object giving the spectral range, on wich the baselines should be evaluated.\\
...
\begin{center}
  ...
  of a spectrum consisting only of noise and baseline:  without giving \Rcode{noise} the resulting baseline
