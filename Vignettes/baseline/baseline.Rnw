...
bl <- spc.fit.poly (chondro [1,, c(630, 1775)], chondro [1])
plot (chondro [1], plot.args = list (ylim = c(250, 750)))
plot (chondro [1,, c(630, 1775)], add = TRUE, lines.args = list (type = "p", pch = 20))
plot (bl, add = TRUE)
@
\begin{figure}[bh]
\begin{center}
...
This is done by excluding spectral regions that contain signals from the baseline fitting. The idea is that all data points that lie above a fitted polynomial (through the whole spectrum) will be treated as signal and thus be excluded from the baseline fitting. 

The supporting points for the baseline polynomials are therefore calculated iteratively:
\begin{enumerate}
