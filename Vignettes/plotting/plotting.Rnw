...
if (!require (latticeExtra)){
  panel.levelplot.points <- function (...) 
    panel.levelplot (...)
  warning ('package latticeExtra required, panel.levelplot.points replaced by dummy')
}
...
\begin{longtable}{@{}p{\plotcol} p{\textcol}}
  \plottab{3D plots with \Rpackage{rgl}}{3D} \Rpackage{rgl}\Sexpr{cite.pkg("rgl")} offers fast 3d
...
@
<<echo = FALSE>>=
load ("par3d.Rdata") # original in Vignettes/laser
par3d (pars)
...
\begin{longtable}{@{}p{\plotcol} p{\textcol}}
  \plottab{plot ("spc") with \Rpackage{longdf}}{ggplotspc} 
<<ggplotspc, fig = TRUE, width = 4, height = 2.6,include = FALSE >>=
df <- as.long.df (flu, rownames = TRUE)
p <- ggplot (df, aes (x = .wavelength, y = spc, 
...
beforehand. Moreover, depending on the particular plot \Rfunction{as.data.frame} may be more suitable
than \Rfunction{as.long.df}:
\begin{longtable}{@{}p{\plotcol} p{\textcol}}
  \plottab{Map with \Rpackage{ggplot2}}{ggplotmeansd} 
<<ggplotmeansd, fig = TRUE, width = 4, height = 2.6,include = FALSE >>=
df <- as.long.df (apply (chondro, 1, mean))
print (object.size (df))
p <- ggplot (df,aes (x = x, y = y, fill = spc)) +
  geom_tile() + 
...
Cut axes can be simulated by factetting. A factor is needed that indicates the respective ranges:
<<ggplotspccut, fig=TRUE, width = 4, height = 2.6,include = FALSE >>=
df <- paracetamol [,, c( min ~ 1800, 2800 ~ max)]
df <- as.long.df (df)
...
<<cleanup, echo = FALSE>>=
make.bib (c("latticeExtra", "rgl", "ggplot2", "playwith", "latticist", "plotrix", "deldir", "tripack"), file = "plotting-pkg.bib")
sessionInfo ()
rm (list = ls ())
@
