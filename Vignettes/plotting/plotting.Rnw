...
\newcommand{\plottab}[2]{
\multicolumn{2}{l}{\textbf{#1} \vspace*{2ex}}\tabularnewline\nopagebreak
\vtop{%
  \vskip-\baselineskip
  \hbox{%
...
\item[\Rfunction{levelplot}] \chy has a levelplot method
\vspace*{1ex}
...
\item[\Rfunction{plotmap}] more specialized version of levelplot for map or image plots.\\
    \Rfunction{plotmap} is a \Rpackage{lattice} function
\item[\Rfunction{plotvoronoi}] more specialized version of plotmap that produces Voronoi tesselations.\\
...
plots. To plot ... 
\begin{longtable}{@{}p{\plotcol} p{\textcol}}
\plottab{particular wavelength range}{wavelength}
...
\\
\plottab{more wavelength ranges}{wavelength-2}
...
plotspc (paracetamol, 
         ...
         xoffset = 750)
@
If available, the package \Rpackage{plotrix} is used to produce the cut mark.
\\
\plottab{with reversed abscissa}{abscissa}
...
@
\\
\plottab{more spectra into an exsisting plot}{add}
...
\Rfunarg{stacked = \emph{factor}}. Alternatively, the name of the grouping extra data column can be
used:
...
\verb+.wavelength+.
\\
\plottab{Condidioning: plotting more traces separately}{plotc2}
...
\Rcode{plotmap} uses \Rcode{levelplot}, a \Rpackage{lattice} function.  Therefore, in loops,
functions, Sweave chunks, etc. the lattice object needs to be printed explicitly by \Rcode{print
  (plotmap (object))}
...
\begin{longtable}{@{}p{\plotcol} p{\textcol}}
 \plottab{plotting clusters}{plotmap-clu}
...
plotmap (chondro.preproc [, , c( 728, 782, 1098, 1240, 1482, 1577)],
         col.regions = matlab.palette)
@ 
\\
...
@ 
\\
\plottab{Conditioning on \Rcode{.wavelength}}{plotmap-pca}
\Rfunction{plotmap} automatically applies the function in \Rfunarg{func} before plotting. This defaults to the \Rfunction{mean}. In order to suppress this, use \Rcode{func = NULL}. This allows conditioning on the wavelengths.
...
@ 
\\
\plottab{Conditioning on \Rcode{.wavelength} II}{plotmap-pca-2}
Alternatively, use \Rfunction{levelplot} directly:
<<plotmap-pca-2, fig = TRUE, width = 4, height = 2.6,include = FALSE >>=
...
@
\\
\plottab{Mark missing spectra I}{missing} 
If the spectra come from a rectangular grid, missing
...
plotmap (sample (chondro, 850), col.regions = matlab.palette(20),
         panel = mark.missing)
@ 
\end{longtable}
...
\begin{longtable}{@{}p{\plotcol} p{\textcol}}
  \plottab{3D plots with \Rpackage{rgl}}{3D} \Rpackage{rgl} offers fast 3d plotting in R. As
  \Rpackage{rgl}'s axis annotations are sometimes awkward. They may better be set manually:
<<>>=
library (rgl)
@
...
\Rcode{plotmap} and \Rcode{plotc} use \Rcode{levelplot}, a \Rpackage{lattice} function.  Therefore,
in loops, functions, Sweave chunks, etc. the lattice object needs to be printed explicitly by
...
external print command, see \Rcode{vignettes.defs}.

\section{Interactive Graphics}
\label{sec:interactive-graphics}

\phy offers two basic interaction functions, \Rfunction{spc.identify}, and
\Rfunction{map.identify}. They identify points in spectra plots and map plots, respectively.
...
function, a preceeding call to \Rfunction{trellis.focus} is needed:
<<eval = FALSE>>=
plot (laser, "mat")
trellis.focus ()
grid.locator ()
...
\phy's lattice functions work with \Rpackage{playwith} and \Rpackage{latticist}. These packages allow
easy customization of the plots and also identification of points.
