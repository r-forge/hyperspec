...
\subsection{Linear Calibration plots}
...
Plotting the Intensities over the concentration for univariate calibration. 

Preparing a hyperSpec object with relevant data:
<<>>=
flu$file <- NULL
flu <- flu[,,450]
...
<<>>=
int <- list (spc = as.matrix (seq (min (flu), length.out = 25) ) )
...
@
Calculated confidence intervals are appended:
<<>>=
tmp <- new ("hyperSpec", 
...
% This is done by  \Rfunction{levelplot}, or by using \Rcode{plot (object, "mat", model = spc ~ % Y.wavelength * other.data.column)}. The actual plotting is done by \Rfunction{levelplot}, so the plots % can be grouped or conditioned.
