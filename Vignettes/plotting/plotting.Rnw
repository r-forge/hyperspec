...
dummies <- check.req.pkg ("latticExtra", ploterrors = c ("panel.levelplot.points"), 
                          special = list (plotvoronoi = NA))
for (i in seq_along (dummies))
  eval (dummies [[i]])

check.req.pkg ("deldir", special = list (plotvoronoi = NA))

assignInNamespace (x = "plotvoronoi", 
                   value = function (...) ploterrormsg ("plotvoronoi", "latticeExtra and deldir"), 
                   ns = "hyperSpec")
...
@
\end{labeling}
In addition \Rpackage{tripack}, \Rpackage{playwith}, and \Rpackage{latticist} are discussed, but not
...
<<ggplotspccut>>
  ...
  for (f in c ("ggplotspc", "ggplotmeansd", "ggplotspccut")){
