...
.spc.filehdr <- function (raw.data) {
  ...
  hdr <- list (ftflgs   = readBin          (raw.data [        1], "integer", 1, 1, signed = FALSE),
               ...
               fres     = paste (rawToChar (raw.data [ 37 :  45], multiple = TRUE), collapse = ""),
               fsource  = paste (rawToChar (raw.data [ 46 :  54], multiple = TRUE), collapse = ""),
               fpeakpt  = readBin          (raw.data [ 55 :  56], "integer", 1, 2, signed = FALSE),
               fspare   = readBin          (raw.data [ 57 :  88], "numeric", 8, 4                ),
               fcmnt    = paste (rawToChar (raw.data [ 89 : 218], multiple = TRUE), collapse = ""),
               fcatxt   = paste (rawToChar (raw.data [219 : 248], multiple = TRUE), collapse = ""),
               ...
               fmethod  = paste (rawToChar (raw.data [265 : 312], multiple = TRUE), collapse = ""),
  ...
  hdr$ftflgs <- read.spc.ftflags (hdr$ftflgs)
  ...
  if (hdr$ftflgs ['TXYXYS'] && hdr$ftflgs ['TMULTI']){
    hdr$subfiledir <- hdr$fnpts - 1 # all file positions are of the last byte before reading starts!
    hdr$fnpts <- 0
  } else {
    hdr$subfiledir <- 0
  }
...
.spc.subfiledir <- function (raw.data, pos, nsub) {
  ...
  dir$ssfposn <- dir$ssfposn - 1 # use positions to mark last read position rather than first to read
  dir
}
...
read.spc <- function (filename,
  ...
  if (! hdr$ftflgs ['TXYXYS']) {
    if (! hdr$ftflgs ['TXVALS']) {
      ...
      if (! hdr$ftflgs ['TMULTI']) {
        dummy <- .spc.read.x (f, fpos, hdr$fnpts)
        wavelength <- dummy$x
        fpos <- dummy$.last.read
      }
    }
  } 
  ...
  log.long <- list (.filename = filename)
  data <- list (z = NA, z.end = NA)
  if (hdr$fwplanes > 0)
    data <- c (data, w = NA)

  ## process the log block
  dummy <- .spc.log (f, hdr$flogoff,
                     log.bin, log.disk, log.txt,
                     keys.log2data,  keys.log2log)
  data <- c (data, dummy$extra.data, .getbynames (hdr, keys.hdr2data))
  data <- as.data.frame (data)
  
  log <- modifyList (list (short = "read.spc",
                           long = c(dummy$log.long,
                             .getbynames (hdr, keys.hdr2log))),
                     log)
...
#               label = label)
  ...
  if (hdr$subfiledir){ ## TXYXYS
    ...
    for (s in seq_len (hdr$fnsub)) {
      ...
      dummy <- .spc.read.y (f, fpos, npts = hdr$subhdr$subnpts, exponent = hdr$subhdr$subexp,
                            word = hdr$ftflgs ['TSPREC'])
      fpos <- fpos + dummy$.last.read
  ...
  if (hdr$ftflgs ['TXYXYS'] && hdr$ftflgs ['TMULTI']) 
    ...
    new ("hyperSpec",  spc = matrix (NA, nrow = hdr$fnsub, ncol = hdr$fnpts), wavelength = wavelength,
...
read.spc.KaiserMap <- function (files, keys.log2log = FALSE,
                                keys.log2data = FALSE, glob = TRUE, ...) {
  if (glob)
  ...
  for (f in seq_along (files)){
    dummy <- read.spc (files [f], keys.log2data = keys.log2data, keys.log2log = keys.log2log,
                       no.object = TRUE)

    data [f, 'x'] <- factor2num (dummy$data$Stage_X_Position)
    data [f, 'y'] <- factor2num (dummy$data$Stage_Y_Position)
    data [f, 'z'] <- factor2num (dummy$data$Stage_Z_Position)
    
    spc$spc  [f, ] <- dummy$spc
  }

  new ("hyperSpec", wavelength = spc$wavelength, spc = spc$spc, data = data, label = spc$label,
       log = spc$log)
}
