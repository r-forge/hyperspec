panel.levelplot <- function (x, y, z, subscripts, at = pretty(z), shrink, labels = FALSE, 
    ...
    border = "transparent", ..., col.regions = regions$col, alpha.regions = regions$alpha,
                             col.mixer = level.colors) 
{
    ...
    y <- as.numeric(y)
    z <- as.numeric(z)
    zcol <- col.mixer (z, at, col.regions, colors = TRUE)
    x <- x[subscripts]
    y <- y[subscripts]
    ...
    if (region) 
        grid.rect(x = cx[idx], y = cy[idy], width = lx[idx] * 
            ...
            height = ly[idy] * scaleWidth(z, shrinky[1], shrinky[2], 
                ...
                lwd = 1e-05, col = border, alpha = alpha.regions))
    if (contour) {
