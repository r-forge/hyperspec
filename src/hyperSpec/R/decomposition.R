...
##’ Object
##’ Decomposition of the spectra matrix is a common procedure in chemometrix
##’ data analysis. \code{decomposition} converts the result matrices into new
##’ \code{hyperSpec} objects.
##’ 
##’ Multivariate data are frequently decomposed by methods like principal
##’ component analysis, partial least squares, linear discriminant analysis,
##’ and the like.  These methods yield loadings (or latent variables,
##’ components, \dots{}) that are linear combination coefficients along the
...
##’ 
##’ The obtained loadings are spectra-like objects: a loading has a coefficient
##’ for each wavelength. If such a matrix (with the same number of columns as
##’ \code{object} has wavelengths) is given to \code{decomposition}, the
##’ spectra matrix is replaced by \code{x}. Moreover, all columns of
##’ \code{object@data} that did not contain the same value for all spectra are
##’ set to \code{NA}.  Thus, for the resulting \code{hyperSpec} object,
##’ \code{\link{plotspc}} and related functions are meaningful.
##’ \code{\link[hyperSpec]{plotmap}} cannot be applied as the loadings are not
##’ laterally resolved.
##’ 
##’ The scores-matrix needs to have the same number of rows as \code{object}
##’ has spectra. If such a matrix is given, the spectra matrix is replaced by
##’ \code{x} and \code{object@wavelength} is replaced by \code{wavelength}. The
##’ information related to each of the spectra is retained. For such a
##’ \code{hyperSpec} object, \code{\link{plotmap}} and \code{\link{plotc}} and
##’ the like can be applied. Of couse, it is also possible to use the spectra
##’ plotting, but the interpretation is not that of the spectrum any longer.
##’ 
##’ @param object A \code{hyperSpec} object.
##’ @param x matrix with the new content for \code{object@data$spc}.
##’ 
##’ May correspond to rows (like a scores matrix) or columns (like a loadings
