...
##’ 
##’ Using \code{na.rm = TRUE} always takes about twice as long as \code{na.rm =
##’ FALSE}.
##’ 
##’ If the spectra matrix does not contain too many \code{NA}s, \code{na.rm =
##’ 2} is faster than \code{na.rm = TRUE}.
##’ 
...
##’ 
##’ if \code{2}, the binning is done using \code{na.rm = FALSE}, and resulting
##’   \code{NA}s are corrected with \code{mean(\dots{}, na.rm = TRUE)}.
##’ @param short,user,date handed to \code{\link[hyperSpec]{logentry}}.
##’ @return A \code{hyperSpec} object with \code{ceiling (nwl (spc) / by)} data
##’   points per spectrum.
##’ @author C. Beleites
