...
##’ These Methods allow to extract and replace parts of the \code{hyperSpec}
##’ object.
##’ 
##’ They work with respect to the spectra (rows of \code{x}), the columns of
##’ the data matrix, and the wavelengths (columns of the spectra matrix).
##’ 
##’ Thus, they can be used for selecting/deleting spectra, cutting the spectral
##’ range, and extracting or setting the data belonging to the spectra.
##’ 
##’ Convenient shortcuts for access of the spectra matrix and the
##’ \code{data.frame} in slot \code{data} are provided.
##’ 
##’ \emph{Extracting: \code{[}, \code{[[}, and \code{$}}.
##’ 
##’ The version with single square brackets (\code{[}) returns the resulting
##’ \code{hyperSpec} object.
##’ 
##’ \code{[[} yields \code{data.frame} of slot \code{@data} of that
##’ corresponding \code{hyperSpec} object returned with the same arguments by
##’ \code{[} if columns were selected (i.e. \code{j} is given), otherwise the
##’ spectra \code{matrix} \code{x@data$spc}.
##’ 
##’ \code{$} returns the selected column of the \code{data.frame} in slot
##’ \code{@data}.
##’ 
##’ \emph{Shortcuts.} Three shortcuts to conveniently extract much needed parts
##’ of the object are defined:
##’ 
##’ \code{x[[]]} returns the spectra matrix.
##’ 
##’ \code{x$.} returns the complete slot \code{@data}, including the spectra
##’ matrix in column \code{$spc}, as a \code{data.frame}.
##’ 
##’ \code{x$..} returns a \code{data.frame} like \code{x$.} but without the
##’ spectra matrix.
##’ 
##’ \emph{Replacing: \code{[<-}, \code{[[<-}, and \code{$<-}}. \preformatted{
##’ ## S4 method for signature 'hyperSpec': x[i, j, l, short = NULL, \dots] <-
##’ value
##’ 
##’ ## S4 method for signature 'hyperSpec': x [[i, j, l, wl.index = FALSE,
##’ short = NULL, \dots]] <- value
##’ 
...
##’   [[<-,hyperSpec-method [<-,hyperSpec-method $<-,hyperSpec-method
##’ @docType methods
...
##’ @param j,name selecting columns of \code{x@data}
...
##’ @param \dots ignored
##’ @return For \code{[}, \code{[<-}, \code{[[<-}, and \code{$<-} a
##’   \code{hyperSpec} object,
##’ 
...
##’ 
##’ \code{x$..} returns the \code{data.frame} in \code{@data} but without the
##’   column \code{@data$spc} containing the spectra matrix.
