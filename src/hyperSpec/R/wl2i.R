...
##’ \code{wl2i} returns the column indices for the spectra matrix for the given
##’ wavelengths.
##’ 
##’ \code{i2wl} converts column indices into wavelengths.
##’ 
##’ If \code{wavelength} is numeric, each of its elements is converted to the
##’ respective index.  Values outside the range of \code{x@wavelength} become
##’ \code{NA}.
##’ 
...
##’ 
##’ is returned. If the wavelengths are not ordered, that may lead to chaos. In
##’ this case, call \code{\link[hyperSpec]{orderwl}} first.
##’ 
##’ Two special variables can be used: \code{min} and \code{max}, corresponding
##’ to the lowest and highest wavelength of \code{x}, respectively.
##’ 
##’ start and end may be complex numbers. The resulting index for a complex x
##’ is then
##’ 
##’ index (Re (x)) + Im (x)
##’ 
...
##’ @return A numeric containing the resulting indices and wavelengths,
##’   respectively.
##’ @author C. Beleites
##’ @examples
##’ 
