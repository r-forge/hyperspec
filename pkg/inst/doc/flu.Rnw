...
Vignette for the Data Set \Rfunction{flu} of the R package \Rpackage{hyperSpec}}
...
The actual calibration is a linear model, which can be fitted by dhe R function \Rfunction{lm}.
...
There are three abbrevations that help to get the parts  of the \Rclass{hyperSpec} object that are frequently  needed:
\begin{description}
