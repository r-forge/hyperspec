...
\Rfunction{colnames}, \Rfunction{rownames}, \Rfunction{dimnames}           & names of the spectra, data columns, and both plus the names of the wavelengths\\
...
\Rfunction{> < == >= <=}                                                   & comparison operators work on \Rcode{\$spc}\\
...
\Rfunction{cbind}, \Rfunction{rbind}                                       & combine two objects by columns or rows\\
...
\Rfunction{sweep}                                                          & ``sweep'' a sum characteristic over rows or columns, e.\,g. center the data by substracting the mean spectrum.\\
\Rfunction{as.character}, \Rfunction{as.matrix}, \Rfunction{as.data.frame} & type conversion functions\\
\Rfunction{initialize}                                                     & create an object\\
...
\caption{\label{fig:structure}The stucture of the data in a \Rpackage{hyperSpec} object.}
\end{figure}
...
\section{Combining \Rclass{hyperspec} Objects \marginpar{\Rfunction{cbind} \Rfunction{rbind}}}
...
\subsection{ASCII Files \marginpar{\Rfunction{read.txt.long} \Rfunction{read.txt.wide} \Rfunction{write.txt.long} \Rfunction{write.txt.wide}}}
...
optimized import function is already available: \Rfunction{scan.txt.Renishaw}. \marginpar{\Rfunction{scan.txt.Renishaw}}
...
\subsection{Wavelength Indices\marginpar{\Rfunction{wl2i} \Rfunction{i2wl}}}
...
The imaginary part is added to the index calculated from the wavelenght
in the real part:
...
\section{Accessing Parts of the \Rclass{hyperSpec} Object\marginpar{\Rfunction{[[]] \$. \$..}}}

\Rclass{hyperSpec} comes with three abbrevation functions for easy
access to the data:
\begin{description}
\item [{\Rfunction{[[]]}] returns the spectra matrix (\Rcode{\$spc}).
\item [{\Rfunction{[[\Rfunarg{i}, , \Rfunarg{l}]]}] the cut spectra matrix is returned if wavelengths are specified in \Rfunarg{l}.
...
\item [{\Rcode{plotmap}}] plots a false color map: a single value (e.\,g. average intensity or cluster membership) over two data columns (default \Rcode{\$x} and \Rcode{\$y}).
...
All three plus some more handy abbrevations are also accessible via \Rmethod{plot}: \marginpar{\Rfunction{plot}}
...
\subsection{Plotting Spectra\marginpar{\Rfunction{plotspc}}}
...
\subsection{Calibration Plots, (Depth) Profiles, and Time Series Plots\marginpar{\Rfunction{plotc}}}
...
\subsection{Plotting False-Color Maps \marginpar{\Rfunction{plotmap}}}
...
\subsection{Cutting the Spectral Range \marginpar{\Rfunction{[]} \Rfunction{[[]]}}}
...
\subsection{Spectral Interpolation and Smoothing \marginpar{\Rfunction{spc.bin} \Rfunction{spc.loess}}}
...
\subsection{Spectral Arithmetic \marginpar{\Rfunction{+ - {*} / \textasciicircum{} log log10}}}
...
In this case, do not forget to adapt the label:\marginpar{\Rfunction{labels}}
<<label, eval = FALSE>>=
...
Matrix multiplication is done by \Rfunction{\%{*}\%} \marginpar{\Rfunction{\%{*}\%}},
...
to each of the groups of spectra specified in \Rfunarg{by}. \marginpar{\Rfunction{aggregate}}
