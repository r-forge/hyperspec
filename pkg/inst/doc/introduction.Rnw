...
cases, direct access to the slots can considerably speed up calculations, see section
...
\subsection{Abbreviations for Parts of the \Rclass{hyperSpec} Object}
...
available.  To keep all spectra replacing missing parts by \Rcode{NA}:
<<>>=
...
\begin{footnotesize}
<<echo = FALSE>>=
make.bib (c ("baseline"), file = "introduction-pkg.bib")
