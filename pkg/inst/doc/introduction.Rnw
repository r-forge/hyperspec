...
\end{labeling}
However, it is good practice to use the functions provided by \Rpackage{hyperSpec} to handle the
...
The second argument of the extraction functions \Rfunction{[]} and  \Rfunction{[[]]} specifies the (extra) data columns. They can be given like any column specification for a \Rclass{data.frame}, \ie numeric, logical, or by a vector of the column names:
...
If different spectra need to be offset by different shift, use a loop\footnote{\Rfunction{sweep}
  cannot be used hare, and while there is the possibility to use sapply or mapply, they are not
...
Figure~\ref{fig:fit-shift} shows that the second correction method works better for the chondrocyte data. This was expected, as the spectra are hardly or not oversampled, but are very similar to each other.
\begin{figure}[t]
  \centering
...
\Rpackage{hyperSpec} provides two functions to do so: \Rfunction{spc.bin} and \Rfunction{spc.loess}.
...
@
If the offset is calculated by a function, as here with the \Rfunction{min}, \chy's \Rfunction{sweep} method offers a shortcut: \Rfunction{sweep}'s \Rfunarg{STATS} argument may be the function instead of a numeric vector:
<<ofs2>>=
...
\mFun{\Rfunction{spc.fit.poly} \Rfunction{spc.fit.poly.below}}\Rfunction{spc.fit.poly} fits a polynomial baseline of the given
...
\mFun{\Rfunction{sweep}} This means that for each of the wavelengths the same correction needs
...
\mFun{\Rfunction{apply} \Rfunction{sweep}} Again, \Rfunction{sweep} is the function of choice. E.\,g. for area normalization, use:
<<normalize1>>=
...
correction, then normalize using \Rfunction{max}.
...
sample? In this case it may be better to subtract the minimum
...
Splitting can be reversed by \Rfunction{rbind}\mFun{\Rfunction{rbind}} (see section~\ref{sec:bind}, page~\pageref{sec:bind}).
Another, similar way to combine a number of \chy objects with different wavelength axes or extra data columns is \Rfunction{collapse}\mFun{\Rfunction{collapse}} (see section~\ref{sec:collapse}, page~\pageref{sec:collapse}).
