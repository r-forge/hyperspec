...
\Rfunction{chk.hy} is not exported by \phy. 
If functions are to be tested outside \phy's namespace
<<>>=
chk.hy <- hyperSpec:::chk.hy
@
makes it accessible in the workspace.
...
Slot \Rcode{@label} contains an element for each of the columns in \Rcode{@data} plus one holding the label for the wavelength axis, \Rcode{.wavelength}. The elements of the list may be anything suitable for axis annotations, \ie they should be either character strings or expressions for ``pretty'' axis annotations (see figure~\ref{fig-spcloess} on page~\pageref{fig-spcloess}).  To get familiar with expressions for axis annotation, see \verb+? plotmath+ and \verb+demo (plotmath)+.
