...
\subsection{Binding Objects that do not share the same extra data and/or wavelength axis}\mFun{\Rfunction{collapse}}
\label{sec:collapse}
...
\subsection{Matrix Multiplication}\mFun{\Rfunction{\%*\%}}
\label{sec:matmult}
