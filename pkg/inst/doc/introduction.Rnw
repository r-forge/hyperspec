...
\Rfunction{hyperSpec.setOptions}.  Currently, the only option provided is \Robject{log}, a logical
specifying whether assignment functions should automatically add entries to the logbook (see
\ref{sec:structure-chy} and ).
...
@
which can be applied to all spectra.
