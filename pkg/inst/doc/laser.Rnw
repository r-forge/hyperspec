...
nice.strip <- function (which.panel, factor.levels, ...){
  lvl <-  factor.levels [which.panel]
  
  lvl <- bquote (lambda == .(lvl) * " nm")
  panel.rect (0, 0, 1, 1, col = "grey90")
  panel.text (0.5, .5, lvl)
}

plotc (laser [,, c (13, 17, 21, 23), wl.index = TRUE], spc ~ t | .wavelength, 
       type = "b", cex = .5,  strip = nice.strip, col = "black")
@ 
\begin{figure}[tbh]
  \centering
...
plot (laser, "mat", col.regions = rev (gray (seq (0.5, .9, length.out = 20))), 
      contour = TRUE, col = "#00000060")
@

\phy's \Rfunction{levelplot} method can be used to display the spectra matrix over a data column (instead of the row number)). Note that the \chy object is the \emph{second} argument to the function (according to the notation in \Rfunction{levelplot}).
<<plotmatt, fig = TRUE, include = FALSE>>=
levelplot (spc ~ .wavelength * t, laser, col.regions = matlab.palette (20), 
           contour = TRUE, col ="#00000080")
@
\begin{figure}[tbh]
  \centering
...
\chy objects  can easily be drawn with \Rpackge{rgl}\cite{rgl}:
...
@ 
\Rpackge{rgl}'s \Rfunction{persp3d} plots a surface in 3d defined by points in x, y, and z. Handing over the appropriate data columns of the \chy object is easy (fig.~\ref{fig:laser3d}):
...
@ 
After nicely rotating and zooming, the parameters to reconstruct this view can be saved:
<<eval = FALSE>>=
pars <- par3d (no.readonly=TRUE)
save (pars, file = "par3d.Rdata")
@
and later applied to a replay of the scene:
<<>>=
load ("par3d.Rdata")                    
par3d(pars)
@

The snapshot of the scene in fig.~\ref{fig:laser3d} is saved by:
<<>>=
rgl.snapshot("fig/laser3d.png", fmt="png", top=TRUE )
@
\begin{figure}[t]
  \centering
  \includegraphics[width = .75\textwidth]{fig/laser3d}  
