...
<<rgl-plot>>
  par3d (userMatrix = matrix (c (-0.52,  0.4, -0.75, 0, 
         ...
         zoom = 0.75)
                                        # producing the snapshot
warning ("par3d x done")
rgl.snapshot ("fig/fig-3D.png", fmt="png", top=TRUE )
                                        # producing the snapshot
warning ("snapshot x done")
rgl.quit() 
                                        # producing the snapshot
warning ("rgl.quit done")
} else {
  png ("fig/fig-3D.png")
  ploterrormsg ("", "rgl")
  dev.off ()
