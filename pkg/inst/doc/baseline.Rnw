...
\Rfunction{spc.fit.poly.below} tries to find appropriate spectal regions to fit the baseline to.
...
Substracting the baseline is up to the user, it is easily done as \chy provides the \Rcode{-} (minus)
operator.
...
As \Rfunction{spc.fit.poly} fits to the whole spectral region of \Rfunarg{fit.to}.  Thus, the spectra
need to be cut to appropriate wavelength ranges that do not contain any signal.
...
cols <- matlab.dark.palette (10)
  #as (HSV(seq (240, 0, length.out = 10), 1, 1), "RGB")
#cols@coords [,2] <- cols@coords[,2] / 2
#cols <- hex (cols)
bl <- chondro [1] + 15
plot (chondro [1])
