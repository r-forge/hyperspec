...
This document discusses baseline correction methods that can be used with \phy. \phy provides two fitting functions for polynomial baselines, \Rfunction{spc.fit.poly} and \Rfunction{spc.fit.poly.below}.
...
The main difference between the two functions is that \Rfunction{spc.fit.poly} returns a least squares fit through the complete spectrum that is given in \Rfunarg{fit.to} whereas  \Rfunction{spc.fit.poly.below} tries to find appropriate spectal regions to fit the baseline to.
...
Both functions fit the polynomial to the spectral range given in \chy object \Rfunarg{fit.to}. If \Rfunarg{apply.to} is not \Rcode{NULL}, the polynomials are evaluated on the spectral range of \Rfunarg{apply.to}. Otherwise, the polynomial coefficients are returned. 

Substracting the baseline is up to the user, it is easily done as \chy provides the \Rcode{-} (minus) operator.

\subsection*{Fitting polynomial baselines using least squares }
\label{sec:fitt-polyn-basel}

Commonly, baselines are fit using (single) support points that are specified by the user. Also, usually $n + 1$ support point is used for a polynomial of order $n$. 
This approach is appropriate for spectra with high signal to noise ratio. 

Such a baseline can be obtained by restricting the spectra in \Rfunarg{fit.to} to the respective points (see figure~\ref{fig:classical}):
...
\begin{center}
  ...
  \caption{Fitting a linear baseline through two points. If the signal to noise ratio is not ideal, wavelengths that work fine for one spectrum (black) may not be appropriate for another (red).}
  \label{fig:classical}
\end{center}
\end{figure}
...
Both  \Rfunction{spc.fit.poly} and \Rfunction{spc.fit.poly.below} therefore provide least squares fitting for the polynomial.

As  \Rfunction{spc.fit.poly} fits to the whole spectral region of \Rfunarg{fit.to}. 
Thus, the spectra need to be cut to appropriate wavelength ranges that do not contain any signal.

In order to speed up calculations, the least squares fit is done by using the Vandermonde matrix and solving the equation system by \Rfunction{qr.solve}.
...
\Rfunction{spc.fit.poly.below} tries to automatically find appropriate spectral regions for baseline fitting. 
This is done by excluding spectral regions that contain signals from the baseline fitting. The idea is that all data points that lie above a fitted polynomial (initially through the whole spectrum, then through the remaining parts of the spectrum) will be treated as signal and thus be excluded from the baseline fitting. 

The supporting points for the baseline polynomials are calculated iteratively:
\begin{enumerate}
\item A polynomial of the requested order is fit to the considered spectral range, initially to the whole spectrum given in \Rfunarg{fit.to}
\item Only the parts of the spectrum that lie below this polynomial plus the \Rcode{noise} are retained as supporting points for the next iteration.
\end{enumerate}
...
It is possible to exclude spectral regions that do not contribute to
the baseline from the fitting, while the baseline is used for the whole
spectrum. This selection of appropriate spectral regions is essential for \Rfunction{spc.fit.poly}. But also \Rfunction{spc.fit.poly.below} can benefit from narrower spectral ranges: the fitting gains speed. The default value for \Rfunarg{npts.min} depends on the number of data points per spectrum. Thus one should consider using more support points than the default value suggests.
<<fit-apply>>=
