...
% \VignetteDepends{latticeExtra, tripack, deldir, rgl, ggplot2, playwith}
...
@
\end{labeling}
In addition \Rpackage{tripack} and  \Rpackage{playwith} are mentioned, but not
...
\phy's lattice functions work with \Rpackage{playwith}\Sexpr{cite.pkg("playwith")} which allows easy customization of the plots and also identification of points.
