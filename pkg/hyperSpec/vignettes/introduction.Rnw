...
For the special case of area normalization using the \code{mean} spectra, the factors can be more conveniently calculated by
<<norm-rowmeans>>=
factors <- 1 / rowMeans (chondro [, , 1600 ~ 1700])
@

and instead of \code{sweep} the arithmetic operators (here \code{*}) can be used directly with the normalization factor: 
